...
const PRESETS = {
  sunset: {
    ...
    fill: { color: 0x8fa0c4, intensity: 0.25 },
    windowGlow: { color: 0xffb070, intensity: 1.15 },
...
export function createTimeOfDay({
  ...
  const sky = room.getObjectByName('sunsetSky')
  const sunDisc = room.getObjectByName('sunsetSun')
  ...
  function applySky(presetName) {
    if (sky?.material) {
      sky.material.map = textures[presetName]
      sky.material.needsUpdate = true
    }
    if (sunDisc) sunDisc.visible = PRESETS[presetName].sunVisible
  }
